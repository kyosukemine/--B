...
def four_envelope(nd,ws=100):
    # ws = windowsize

    y1 = np.array(envelope(nd[:,0],ws))
    y2 = np.array(envelope(nd[:,1],ws))
    y3 = np.array(envelope(nd[:,2],ws))
    y4 = np.array(envelope(nd[:,3],ws))
    nd = np.vstack([y1,y2,y3,y4]).T
    return nd
...
def cut(nd,status,onnso_0,onnso_1,onnso_2,onnso_3,onnso_4,onnso_set,id):
    for i in range(4):# 筋肉の部位をiとして先頭に記録
        # [音素番号，筋肉部位，n_o,voice,muscle,name,id,~]
        j = i
        if status[2] == 2:
            j += 4
...
save_fig_dir = "envelope_only_ws100_ave_on" #フォルダ作成
window_size = 100
