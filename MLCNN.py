...
from keras.layers import Conv1D, MaxPool1D, Input,Conv2D, MaxPool2D
...
def CNNLearn(Learn_num, inputdata, outputdata, validation_data_x, validation_data_y):
    ...
    # モデルの構築
    batchsize = 10                         # バッチサイズ 1,32,128,256,512
    epoch = 50                           # 学習世代
    number_filters_1 = 64                 # フィルター数
    number_filters_2 = 32                 # フィルター数
    kernel_size_1 = (20,4)                    # カーネルサイズ
    kernel_size_2 = (10,4)                    # カーネルサイズ
    pool_size_1 = (1,2)                       # プールサイズ
    pool_size_2 = (1,2)                       # プールサイズ
    stride_1 = (1,1)                          # ストライド数
    stride_2 = (1,1)                          # ストライド数
    ...
    # モデル構造
    input_0 = Input(shape=(4, 1000,1))
    input_1 = Input(shape=(input_data_len, 1))
    input_2 = Input(shape=(input_data_len, 1))
    input_3 = Input(shape=(input_data_len, 1))
    input_4 = Input(shape=(input_data_len, 1))
    input_5 = Input(shape=(input_data_len, 1))
    input_6 = Input(shape=(input_data_len, 1))
    input_7 = Input(shape=(input_data_len, 1))
    x = Conv2D(filters=number_filters_1, kernel_size=kernel_size_1, padding='same', strides=stride_1)(inputs)
    x = Activation(activatioin_func)(x)
    x = MaxPool2D(pool_size=pool_size_1, padding='valid')(x)
    x = Conv2D(filters=number_filters_2, kernel_size=kernel_size_2, padding='same', strides=stride_2)(x)
    x = Activation(activatioin_func)(x)
    x = MaxPool2D(pool_size=pool_size_2, padding='valid')(x)
    x = Flatten()(x)
    ...
    # モデルの保存と後処理
    model.save("./test/Learned_model0.h5")
    # del model
    K.clear_session()
    gc.collect()
...
def main():
    ...
    # np.savetxt("x_train.csv", x_train_copy, delimiter = ",")
    # np.savetxt("x_test.csv", x_test_copy, delimiter = ",")
    # np.savetxt("y_train.csv", y_train_copy, delimiter = ",")
    # np.savetxt("y_test.csv", y_test_copy, delimiter = ",")
    ...
    CNNLearn(1, x_train, y_train, x_test, y_test)
    # モデル読み込み
    model = load_model("./test/Learned_model0.h5", compile=False)
